...
Plus set variable `USE_WHISPER_TRANSCRIBE = True` in the settings.py file. To use VideoCaptioner captions, use the same `docker-compose.full.yaml` override and set `USE_VIDEOCAPTIONER_TRANSCRIBE=True`.
...
Transcription functionality is available only for the Docker installation. To enable this feature, you must either use the `docker-compose.full.yaml` file, as it contains the necessary requirements, or you can also set the celery_worker service to use the mediacms:full image instead of mediacms:latest. Then you also have to set the setting: `USE_WHISPER_TRANSCRIBE = True` in your local_settings.py file. VideoCaptioner subtitle generation also uses the full image; enable it with `USE_VIDEOCAPTIONER_TRANSCRIBE=True` and configure `VIDEOCAPTIONER_ASR` / `VIDEOCAPTIONER_LANGUAGE` if needed.
